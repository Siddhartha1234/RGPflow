...
from gpflow import Parameterized, settings
from gpflow import autoflow, params_as_tensors, Parameter
from gpflow.features import InducingPoints
from gpflow import transforms, DataHolder, Minibatch
...
class Layer(Parameterized):
    """
      A base class for all hidden layers of RGP
    """

    def __init__(self, X, Y, kern, likelihood, mean_function,
                 num_latent, **kwargs):
        """
        :param input_prop_dim: the first dimensions of X to propagate.
        If None (or zero) then no input prop
        :param kwargs:
        """
...
class SVGP_Layer(Layer):
    ...
    def __init__(self, X, Y, kern, likelihood, feat=None,
        ...
        Layer.__init__(self, X, Y, kern, likelihood,
    ...
    def _init_variational_parameters(self, num_inducing, q_mu, q_sqrt, q_diag):
        ...
        """
        q_mu = np.zeros((num_inducing, self.num_latent)
                        ) if q_mu is None else q_mu
        ...
        if q_sqrt is None:
            if self.q_diag:
                ...
                q_sqrt = np.array(
                    [np.eye(num_inducing, dtype=settings.float_type) for _ in range(self.num_latent)])
                self.q_sqrt = Parameter(q_sqrt, transform=transforms.LowerTriangular(
    ...
    def _build_likelihood(self):
        ...
        scale = tf.cast(self.num_data, settings.float_type) / \
            tf.cast(tf.shape(self.X)[0], settings.float_type)
